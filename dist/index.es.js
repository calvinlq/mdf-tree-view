...
var _ = Object.defineProperty, D = Object.getOwnPropertySymbols, W = Object.prototype.hasOwnProperty, U = Object.prototype.propertyIsEnumerable, B = (w, e, n) => e in w ? _(w, e, { enumerable: !0, configurable: !0, writable: !0, value: n }) : w[e] = n, V = (w, e) => {
  for (var n in e || (e = {}))
    W.call(e, n) && B(w, n, e[n]);
  if (D)
    for (var n of D(e))
      U.call(e, n) && B(w, n, e[n]);
  return w;
};
class j {
  constructor(e, n, t, i = {}) {
  ...
  init() {
    this.container.style.height || (this.container.style.minHeight = "500px"), this.container.style.overflow = "auto", this.container.innerHTML = '<svg id="connection-layer" width="100%" height="100%"></svg>';
  ...
  handleNodeClick(e) {
    ...
    if (this.selectedNode && this.selectedNode !== e) {
      ...
      if (this.enableLink && r && l && r.id !== l.id) {
        const u = this.selectedNode, a = e, d = `connection-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, g = { source: u, target: a, id: d };
        this.connections.push(g), this.drawConnections(), this.onConnectionsChange && this.onConnectionsChange([...this.connections]), this.onConnectionChange && this.onConnectionChange(g, "add"), this.selectedNode = null;
        return;
  ...
  findNearestVisibleAncestor(e) {
    ...
    return r.sort((a, d) => {
      const g = a.getBoundingClientRect(), E = d.getBoundingClientRect();
      return Math.abs(g.top - o.top) - Math.abs(E.top - o.top);
    }), r[0].dataset.id;
  }
  ...
  adjustContainerSize() {
    ...
    let n = 0, t = 0, i = 0, s = 0;
    const o = window.getComputedStyle(this.container), r = parseInt(o.width), l = parseInt(o.height), u = parseInt(o.minHeight || "0");
    e.forEach((g) => {
      const E = parseInt(g.style.left || "0"), b = parseInt(g.style.top || "0"), k = g.getBoundingClientRect(), N = k.width, v = k.height;
      n = Math.min(n, E), t = Math.min(t, b), i = Math.max(i, E + N), s = Math.max(s, b + v);
    });
    const a = i - n, d = s - t;
    if (n < 0 || t < 0)
      e.forEach((g) => {
        const E = parseInt(g.style.left || "0"), b = parseInt(g.style.top || "0");
        g.style.left = `${E - n + 20}px`, g.style.top = `${b - t + 20}px`;
      }), this.container.style.width = `${Math.max(a + 40, r)}px`, this.container.style.minHeight = `${Math.max(d + 40, u)}px`;
    else {
      i > r && (this.container.style.width = `${i + 50}px`);
      const g = Math.max(s + 50, u);
      g > l && (this.container.style.minHeight = `${g}px`);
    }
  }
  drawConnections() {
    const e = document.getElementById("connection-layer");
    ...
    if (this.removeDeleteMenu(), this.connections.forEach((t) => {
      ...
      if (i && s) {
        ...
        if (!r) return;
        const l = i.getBoundingClientRect(), u = s.getBoundingClientRect(), a = o.getBoundingClientRect(), d = r.getBoundingClientRect(), g = this.treeContainers.findIndex((m) => m.id === o.id), E = this.treeContainers.findIndex((m) => m.id === r.id), b = g < E, k = this.getNodeVisibilityPosition(t.source), N = this.getNodeVisibilityPosition(t.target);
        let v, C, A, y;
        if (this.isNodeVisible(t.source)) {
          const m = o.getBoundingClientRect();
          v = b ? m.right + 5 - n.left : m.left - 5 - n.left, C = l.top + l.height / 2 - n.top;
        } else {
          const m = document.querySelector(
            `[data-id="${t.source}"]`
          );
          let h = null, f = m.parentElement;
          for (; f && f.classList.contains("tree-children"); ) {
            if (!f.classList.contains("active")) {
              const x = f.parentElement.dataset.id;
              if (this.isNodeVisible(x)) {
                h = x;
                break;
              }
            }
            f = f.parentElement && f.parentElement.parentElement ? f.parentElement.parentElement : null;
          }
          if (h) {
            const x = document.querySelector(
              `[data-id="${h}"] .tree-label`
            ).getBoundingClientRect();
            v = b ? a.right + 5 - n.left : a.left - 5 - n.left, C = x.top + x.height / 2 - n.top;
          } else if (v = b ? a.right + 5 - n.left : a.left - 5 - n.left, k === "top")
            C = a.top - n.top;
          else if (k === "bottom")
            C = a.bottom - n.top;
          else {
            const p = this.findNearestVisibleAncestor(t.source);
            if (p) {
              const x = document.querySelector(
                `[data-id="${p}"] .tree-label`
              ).getBoundingClientRect();
              C = x.top + x.height / 2 - n.top;
            } else
              C = a.top + a.height / 2 - n.top;
          }
        }
        let R = null;
        if (!this.isNodeVisible(t.source)) {
          ...
          for (; h && h.classList.contains("tree-children"); ) {
            if (!h.classList.contains("active")) {
              const p = h.parentElement.dataset.id;
              if (this.isNodeVisible(p)) {
                R = p;
                break;
              }
            }
        ...
        k === "top" && !R && (C = o.getBoundingClientRect().top - n.top);
        let L = null;
        if (this.isNodeVisible(t.target))
          A = b ? d.left - 3 - n.left : d.right + 10 - n.left, y = u.top + u.height / 2 - n.top;
        else {
          ...
          if (L) {
            const p = document.querySelector(
              `[data-id="${L}"] .tree-label`
            ).getBoundingClientRect();
            A = b ? d.left - 5 - n.left : d.right + 5 - n.left, y = p.top + p.height / 2 - n.top;
          } else if (A = b ? d.left - 3 - n.left : d.right + 10 - n.left, N === "top")
            y = d.top - n.top;
          else if (N === "bottom")
            y = d.bottom - n.top;
          else {
            const f = this.findNearestVisibleAncestor(t.target);
            if (f) {
              const p = document.querySelector(
                `[data-id="${f}"] .tree-label`
              ).getBoundingClientRect();
              y = p.top + p.height / 2 - n.top;
            } else
              y = d.top + d.height / 2 - n.top;
          }
        }
        N === "top" && !L ? y = r.getBoundingClientRect().top - n.top : N === "bottom" && !L && (y = r.getBoundingClientRect().bottom - n.top);
        ...
        let $;
        const S = this.bezier;
        b ? ($ = `M ${v} ${C} C ${v + S} ${C}, ${A - S} ${y}, ${A} ${y}`, c.setAttribute("marker-end", "url(#arrowhead-end)")) : ($ = `M ${v} ${C} C ${v - S} ${C}, ${A + S} ${y}, ${A} ${y}`, c.setAttribute("marker-end", "url(#arrowhead-start)")), c.setAttribute("d", $), b ? c.setAttribute("stroke", "#4096ff") : c.setAttribute("stroke", "#ff7a45");
        const I = this.isNodeVisible(t.source), T = this.isNodeVisible(t.target), M = k === "top", q = N === "top";
        c.setAttribute("stroke-width", "1.5"), c.setAttribute("fill", "none"), M || q || !I || !T ? c.setAttribute("stroke-dasharray", "5,5") : c.removeAttribute("stroke-dasharray");
        ...
        }), c.addEventListener("contextmenu", (m) => {
          m.preventDefault(), this.currentConnection = t, this.showDeleteMenu(m.clientX, m.clientY);
        }), e.appendChild(c);
      }
